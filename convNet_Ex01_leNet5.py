...
Description:
	update:
		based on Ex00, added one more hiddenlayer to form a LeNet-5 
...
'''
from Refmlp import HiddenLayer
from Reflogistic_sgd import LogisticRegression, load_data
from Refconvolutional_mlp import LeNetConvPoolLayer
'''
...
class ConvNet(object):
	...
	def __init__(self, randomSeed, 
		...
		hiddenLayer_input_d_2,
		hiddenlayer_output_d_2,
		n_class ):
		'''
		...
		self.symbolic_x = T.matrix('symbolic_x')
		self.symbolic_y = T.ivector('symbolic_y')
		...
		self._layer_hidden2 =  HiddenLayer( randomSeed, self._layer_hidden1.output, 
			n_in = hiddenLayer_input_d_2,
			n_out = hiddenlayer_output_d_2,
			activation = T.tanh )

		self._layer_logistic = LogisticRegression( self._layer_hidden2.output, 
			n_in = hiddenlayer_output_d_2,
			n_out = self._classN )
	...
	def  trainingStart(self, learning_rate, epochN, batch_size, ):#################################################################################
		...
		self._patience = 1000;# run at least these many iterations, every iteration train on "_batchSize" numnber of data
	...
	def _setTrainingData(self, incoming ):
		...
		self._train_data_x, self._train_data_y = incoming
		self._train_data_ready = True

	def _setValidateData(self, incoming ):
		##Phase_II: check type
		self._validate_data_x, self._validate_data_y = incoming
		self._validate_data_ready = True

	def _setTestData(self, incoming ):
		##Phase_II: chech type
		self._test_set_x, self._test_set_y = incoming
...
instanceEx01 = ConvNet( randomSeed = rSeed, 
		filter_shape_1 = ( 20, 1, 5, 5 ), image_shape_1 = ( 500, 1, 28, 28 ), pool_size_1 = ( 2, 2 ), 
		filter_shape_2 = ( 50, 20, 5, 5 ), image_shape_2 = ( 500, 20, 12, 12 ), pool_size_2 = ( 2, 2 ),
		hiddenLayer_input_d = 50*4*4,
		hiddenlayer_output_d = 500,
		hiddenLayer_input_d_2 = 500,
		hiddenlayer_output_d_2 = 300,
		n_class = 10 )
instanceEx01.setData( 'mnist.pkl.gz' )
instanceEx01.trainingStart( learning_rate = 0.1, epochN = 30, batch_size = 500 )
instanceEx01.writeTrainingLocalMinimum( "convRecord_LeNet5.dat" )
print( "... So I take it that the Ex01 finally works." )
